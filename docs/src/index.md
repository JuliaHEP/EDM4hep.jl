...
## PODIO generation
The Julia POD `structs` are generated from the [edm4hep.yaml](https://github.com/key4hep/EDM4hep/blob/main/edm4hep.yaml) and [edm4eic.yaml](https://github.com/eic/EDM4eic/blob/main/edm4eic.yaml) yaml files using [PODIO](https://github.com/AIDASoft/podio) scripts. 

For the time being, for this evaluation, the **Julia structs are generated by a local Julia script** in `./podio/generate.jl`. The files `genComponents_$(edmodel)/jl` and `genDatatypes_$(edmodel).jl` are fully generated for both models. 
...
## Tests
Unit tests can be run with `julia --project -e 'using Pkg; Pkg.test()'`
